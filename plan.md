...
```
lua/mya/
  init.lua          -- setup(), public API, MyaStatusline()
...
Deliverable: `:lua require('mya.agent').get('gemini'):request('initialize', ...)`
...
- `BufReadCmd mya://*`: parse `mya://<agent>/<session>/<view>`, create scratch
...
Deliverable: open `mya://fake/<id>/log`, watch a live turn stream in, fold to
transcript view, jump to a location.

## Phase 4 — dashboard (`ui/dashboard.lua`)

- `:Mya` → scratch buffer listing sessions grouped by agent. On open: ensure
...
- `:{range}Mya send {text}`: builds content blocks — text + optional resource
  block for the range (path, line span, text) or whole files via
  `:Mya send @file ...` args. `:Mya` subcommand completion (`send`, `cancel`,
...
- Turn state in winbar: spinner while `prompting`, key hint while
  `waiting_permission`; `<C-c>`/`:Mya cancel` → `session/cancel`.
...
- Quickfix: `:Mya qf` (and auto on permission, config-gated) fills quickfix
...
- `usage_update` → session.usage; `require('mya').statusline(buf?)` returns
  `model · effort · 42% ctx · gemini · $0.13` with graceful "—" for missing
  fields; winbar auto-set on mya:// buffers (config-gated).
...
- `:checkhealth mya`: nvim version, agents configured, binaries found,
  handshake + capability dump per agent (makes the "setup required" emptiness
  obvious).
- `doc/mya.txt` vimdoc; README with agent recipes (Claude Code, Gemini CLI,
  opencode…).
<!-- - Follow mode (config off by default): tool-call `locations` move cursor in a -->
<!--   designated window. -->
- Log level config + `:Mya log` opening the rpc trace for debugging.
