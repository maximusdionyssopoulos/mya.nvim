...
- Every view is a real buffer served through an `acp://` URL scheme via
  ...
  - `acp://<agent>/<session>/log` — full history: prompts, tool calls, reasoning
  - `acp://<agent>/<session>/transcript` — clean view: prompts & outputs only
  - `acp://<agent>/<session>/plan` — formatted plan, if the session has one
  - `acp://<agent>/<session>/edit/<n>` — a single proposed edit
...
- `:Acp` opens the session dashboard: all sessions across configured agents,
...
- `:{range}Acp send {text}` — range-first, so `'<,'>` visual sends are just one
...
- Expose an `AcpStatusline()` / winbar component function instead of drawing
...
- **Pure Lua**, two layers:
  1. `acp.lua` protocol module — `vim.uv` spawn + stdio pipes, JSON-RPC framing,
...
- Zero-config startup, no mandatory `setup({})`: agents declared in a simple
  table or discovered from `.acp.json`; behavior tuned via native options and
  autocmds.
