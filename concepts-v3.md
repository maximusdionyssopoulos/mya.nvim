...
- Every view is a real buffer served through an `mya://` URL scheme via
  `BufReadCmd`:
  - `mya://<agent>/<session>/log` — the session history
...
- `:Mya` opens the session dashboard. Sessions are fetched live from each
...
- `:{range}Mya send {text}` — range-first, so `'<,'>` visual sends are one
...
- Exposed as an `MyaStatusline()`-style component function users wire into
...
- Standard Lua plugin convention: `require('mya').setup({...})` — agents
...
- Opening `:Mya` requires spawning + initializing every configured agent (or
...
- The plugin does nothing until configured with at least one agent; docs and
  `:checkhealth mya` need to make the empty state obvious.
